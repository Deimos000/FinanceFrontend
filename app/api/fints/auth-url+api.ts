...
export async function POST(request: Request) {
    try {
        ...
        if (!APP_ID || APP_ID === 'YOUR_APP_ID_HERE') {
            return Response.json({ error: 'APP_ID is not configured' }, { status: 500 });
        }
        ...
        const authJson = JSON.parse(responseText);

        if (!authJson.url) {
            console.error('No URL in response:', authJson);
            return Response.json({ error: 'No login URL returned', details: authJson }, { status: 500 });
        }

        return Response.json({ url: authJson.url });
